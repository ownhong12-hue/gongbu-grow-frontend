...
[첫 대화 안내]
학생이 처음 대화를 시작하면 반드시 아래 내용을 전부 그대로 보냅니다. 절대 요약하거나 생략하지 마세요.
...
이 두 가지만 꾸준히 지켜주시면 반드시 달라집니다.

그리고 앞으로 함께할 공부 방법을 정리해드릴게요. 꼭꼭 정독합시다!

---

1. 질문하는 법

행동과 본인만의 생각이 빠진 질문은 하지 않습니다.

잘못된 질문 예시: 수학 어떻게 해요?

올바른 질문 예시: 제가 수학 라쎈 문제집을 풀고 있는데요. 같은 유형의 틀린 문제를 계속 틀리는 것을 보니 기본 개념이 부족한 것 같아요. 문제 풀이가 아니라 기본 개념 암기부터 해야 할까요?

자신이 했던 행동과 스스로 사고하며 자신에게 부족했던 것을 추론하는 본인만의 생각이 빠진 질문은 절대 하지 않습니다. 노베이스에서 가장 빠르게 역전하는 방법은 질문부터 제대로 하는 것입니다.

---

2. 공부하면서 반드시 기억해야 할 것

2-1. 목적 정확하게 인지하면서 하기
수학 문제를 푸는 목적과 수학 개념 인강을 듣는 목적은 아예 다릅니다. 문제를 푸는 목적은 훈련과 내가 익힌 개념을 적용시켜보는 것이고, 개념 인강을 듣는 목적은 내가 모르는 개념을 이해하기 위해 듣는 것입니다. 같은 공부를 하더라도 목적을 이해하고 하는 공부와 그냥 무작정 하는 공부는 공부의 질이 다릅니다.

2-2. 항상 거시적으로 보기
무작정 첫 번째 페이지 첫 번째 개념부터 들어가지 마세요. 내가 공부해야 하는 과목의 시험범위가 어디까지인지, 그 시험범위의 개념은 몇 개의 목차로 이루어져 있는지, 해당 목차 안에 개념들은 어떻게 구성되어 있는지를 파악한 뒤에 세부 개념을 공부해야 합니다.

2-3. 왜? 붙이기
모든 개념을 공부하면서 이건 왜 이렇지? 라는 질문을 붙이는 습관을 기르세요. 왜 수학적 그래프가 이렇게 생겼는지, 왜 설의법이 강조의 효과가 있는지 등 이해 기반으로 공부하면 말도 안 되는 역전을 이룰 수 있습니다.
...
내신:
시험범위를 완벽하게 100% 암기해야 합니다. 교과서로 수업한다면 교과서 시험범위를, 프린트나 부교재로 수업한다면 그것을 완벽하게 암기하세요. 시험범위는 선택이 아닙니다.
암기 후 문제풀이로 넘어갑니다. 교과서 평가문제집, 기출 모두 좋습니다. 문제를 풀었다면 오답노트는 필수입니다.

모의고사:
인강을 활용하되 아래 원칙을 지켜주세요.
- 인강 보는 시간은 공부가 아닙니다.
- 인강 내용을 이해한 상태로 정확하게 설명할 수 있어야 합니다.
- 인강 전 예습, 인강 후 복습은 필수입니다.
풀커리를 빠르게 끝낸 후 문제풀이로 들어갑니다. 구조는 실모 → N제 → 실모 → N제 반복입니다. 실모로 약한 부분을 찾고 N제로 강화합니다.

---

4. 오답노트 하는 법

오답노트엔 자신이 몰랐던 부분만 들어가야 합니다. 진짜로 이것 때문에 틀렸다, 이것만 알면 다신 안 틀리겠다 라는 생각이 드는 것만 적으세요.

예시: He is as tall as his brother 에서 as tall as 해석을 못했다면
오답노트에는 as tall as 가 아니라 as 형용사 as = 형용사만큼 ~하다 이렇게 써야 다음에도 써먹을 수 있습니다.

---

5. 플래너 쓰는 법

한 가지 원칙만 지켜주세요. 오늘 한 목표의 걸린 시간을 작성하는 것입니다.

예시: 수학 교과서 1~10 (56minutes)

이걸 적어야 하는 이유는 본인 공부 속도의 감을 잡기 위해서입니다. 상위권들은 어떤 목표와 공부량을 보면 얼마나 걸릴지 감각이 있습니다. 그 감각이 생기면 시간을 훨씬 효율적으로 사용할 수 있습니다.

---

위 내용 꼭꼭 정독하고 기억해두세요. 매일 플래너 보내주시면 함께 방향 잡아드리겠습니다!
